...
for i in range(len(file_paths_uv)):
    ...
    for i in range(1024):
        for j in range(768):
            ...
            if data0[2,i,j]!=0:
                item=np.where((np.abs(data_i[0]-data0[0,i,j])<0.05)&(np.abs(data_i[1]-data0[1,i,j])<0.05)&(data_i[2]==data0[2,i,j]))
    ...
    save_path = path_uv_i.replace('densepose_fat', 'densepose_fat_cor_new4.0')
    np.save(save_path, c,allow_pickle=True)
    cor.append(c)
cor = np.array(cor)
np.save('/home/nfs/jsh/HOME/VITON-hd-resized/try2.0/cor4.0.npy', cor,allow_pickle=True)
print(cor.shape)
# 10 1024 768 2
# 6-16个点
